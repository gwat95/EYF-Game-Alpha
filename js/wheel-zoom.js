...
WZoom.prototype = {
    ...
    _computePosition(scale, x, y) {
        ...
        if (scale === content.minScale) {
            ...
            contentNewLeft = focusX;
            contentNewTop = focusY;
        }
...
function optionsConstructor(targetOptions, defaultOptions, instance) {
    ...
    if (typeof dragScrollableOptions.onGrab === 'function') {
        options.dragScrollableOptions.onGrab = (event) => {
            if (!instance.isInteractable) return;
            console.log("FUCK YOU");
            dragScrollableOptions.onGrab(event, instance)
        };
    }

    if (typeof dragScrollableOptions.onMove === 'function') {
        options.dragScrollableOptions.onMove = (event) => {
            if (!instance.isInteractable) return;
            dragScrollableOptions.onMove(event, instance)
        };
    }

    if (typeof dragScrollableOptions.onDrop === 'function') {
        options.dragScrollableOptions.onDrop = (event) => {
            if (!instance.isInteractable) return;
            dragScrollableOptions.onDrop(event, instance);
        }
    }
