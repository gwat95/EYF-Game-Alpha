...
Window_Gold.prototype.value = function() {
    return Math.floor($gameParty.gold());
};
...
Window_NameEdit.prototype.drawUnderline = function(index) {
    ...
    this.contents.fillRect(rect.x, rect.y + 15, rect.width, rect.height, color);
...
Window_NameEdit.prototype.drawChar = function(index) {
    ...
    this.resetTextColor();
    let x = rect.x;
    let c = this._name[index];
    if (c === "m" || c === "w") x-= 2;
    this.drawText(this._name[index] || "", x, rect.y + 15);
};

Window_NameEdit.prototype.refresh = function() {
    this.contents.clear();
    this.drawText("Enter your name", 0, 3, this.width, "center");
    ...
    this.setCursorRect(rect.x, rect.y + 15, rect.width, rect.height);
};

Window_NameEdit.prototype.resetFontSettings = function() {
...
Window_NameInput.prototype.setEditWindow = function(editWindow) {
    ...
    document.onkeydown = function(e) {
        GBCInput.lastInput = e.key;
    }
};

Window_NameInput.prototype.isLetter = function(c) {
    return c.toLowerCase() != c.toUpperCase();
  }

Window_NameInput.prototype.consumeInput = function() {
    console.log(GBCInput.lastInput);
    if (GBCInput.lastInput === "ArrowDown") {
        GBCInput.lastInput = null;
        return;
    } else if (GBCInput.lastInput === "ArrowUp") {
        GBCInput.lastInput = null;
        return;
    } else if (GBCInput.lastInput === "ArrowRight") {
        GBCInput.lastInput = null;
        return;
    } else if (GBCInput.lastInput === "ArrowLeft") {
        GBCInput.lastInput = null;
        return;
    } else if (GBCInput.lastInput === "Escape" || GBCInput.lastInput === "Enter") {
        GBCInput.lastInput = null;
        return;
    } else if (GBCInput.lastInput === "CapsLock") {
        this._allCaps = !this._allCaps;
        GBCInput.lastInput = null;
        return;
    } else if (GBCInput.lastInput === "Backspace") {
        GBCInput.lastInput = null;
        this.processBack();
        return;
    } else if ((!this.isLetter(GBCInput.lastInput) && GBCInput.lastInput !== " ") || GBCInput.lastInput.length > 1 ) {
        GBCInput.lastInput = null;
        this.deselect();
        return;
    }
    if (this._allCaps) GBCInput.lastInput = GBCInput.lastInput.toUpperCase();
    console.log(GBCInput.lastInput);
    if (this._editWindow.add(GBCInput.lastInput)) {
        this.playOkSound();
    } else {
        this.playBuzzerSound();
    }
...
Window_NameInput.prototype.character = function() {
    return this._index < 57 ? this.table()[this._page][this._index] : "";
};
...
Window_NameInput.prototype.resetFontSettings = function() {
    this.contents.fontFace = $gameSystem.numberFontFace();
    this.contents.fontSize = $gameSystem.mainFontSize();
    this.resetTextColor();
};
